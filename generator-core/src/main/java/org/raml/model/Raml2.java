...
public class Raml2 extends Raml {
    ...
    public void addGlobalSchema(
            ...
            final String content) {
        String schemaName = fileName.replaceAll("-schema","").replaceAll(".json","").replaceAll(".xsd","");
        schemaMap.put(schemaName, "schemas/" + fileName);
        if (!schemaDeclared(fileName)) {
            schemas.add(createSchemaMap(fileName, content));
        }
    }
